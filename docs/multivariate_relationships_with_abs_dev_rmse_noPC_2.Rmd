...
rel_vars <- c("se_x_w", "se_y_w", "rhos_max", "fungis_max", "y")
  #c("se_x_w", "se_y_w", "fungis_max", "rel_weight", "rel_n_w")
other_vars <- c("rel_weight", "rel_n_w", "sd_emp_inv", "p_hetero", "p_fit_x")
...
all_lm <- targ_both %>% 
  ...
  summarise(
    ...
    se_y_reln = list(lm(abs_dev ~ se_c + poly(y_c, 2, raw = TRUE) + rel_n_w)),
...
- `main_good`: Main effects of the four "good" moderators (SE (combined), correlations, parameter trade-offs, estimate (quadratic)).
