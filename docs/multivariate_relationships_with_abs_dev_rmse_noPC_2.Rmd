...
rel_vars2 <- c("se_x_w", "se_y_w", "rhos_max", "fungis_max", 
               "poly(y_c, 2, raw = TRUE)")

rel_vars3 <- c("se_c", "rhos_max", "fungis_max", 
               "poly(y_c, 2, raw = TRUE)")

all_vars <- c(rel_vars3, "sd_emp_inv", "rel_weight", "rel_n_w",
              "log1p(p_hetero)", "log1p(p_fit_x)")
all_vars2 <- c(rel_vars3[-4], "y_c", "sd_emp_inv", "rel_weight", "rel_n_w",
              "log1p(p_hetero)", "log1p(p_fit_x)")
...
This figure shows that for several of the pairs there is substantial correlation between .25 and .5, but not really higher. 
...
This figure also shows that for several of the pairs there is substantial correlation between .25 and .5 but not really higher.
...
- `se_y_sd`: standard error, value of the estimate, and parameter variability (SD)
...
The results show that the model with all `main` effects usually provides an account that is very similar to the `parameter` model.  Even including interaction does not bring too much, although including all adds many parameters.

The following table shows the additional variable in the model with the best two moderators and the two additional predictors in the model with the best three moderators. The table below the parameters in the model with the best four parameters.
...
The following plot shows the coeffcicients for the model with SE, value (linear and quadratic) and SD
...
pmulti <- ggplot(best3_pars, aes(y = term, x = estimate)) +
  ...
  guides(shape=guide_legend(nrow=2,byrow=TRUE, title = "Method pair"))
...
cors_cmle <- targ_cmle %>% 
  ...
  mutate(rel_weight = log(rel_weight),
         ...
         p_fit_x = log1p(p_fit_x),
         y_c_quad = y_c^2) %>% 
  select(se_c, y_c, y_c_quad, sd_emp_inv) %>%
  nest() %>% 
  summarise(cor = map(data, 
                      ~as_tibble(cor(., use = "na.or"), rownames = "par1"))) %>% 
  mutate(cor = map(cor, ~pivot_longer(., -par1, names_to = "par2", values_to = "cor"))) %>% 
...
cors_both <- left_join(cors_cmle, cors_lpp, 
                       by = c("par1", "par2"), suffix = c(".cmle", ".lpp")) %>% 
  filter(par2 != "se_c", par1 != "sd_emp_inv") #%>% 
  #select(par1:`Comp Bayes.cmle`) 
## filter out all repeating columns
cors_both$corr <- NA_character_
for (i in 1:nrow(cors_both)) {
