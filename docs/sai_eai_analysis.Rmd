...
```{r, fig.width=18, fig.height=5, message=FALSE, eval=TRUE}
...
```{r, fig.width=18, fig.height=5.3, message=FALSE, warning=FALSE}
...
### Selected Pairs: SE < `r MIN_REL_SE`, Rho < `r MIN_REL_RHO`

The following table provides an overview of thirty observations with SE_c < `r MIN_REL_SE` and rho_max < `r MIN_REL_RHO` ordered by `z_abs_dev` which is calculated as `abs_dev`/`se_c`. It is clear that the largest deviations involve NP-Bayes.
...
```{r, fig.width=10, fig.height=3.5, message=FALSE, eval=FALSE}
cur_data %>% 
  filter(rhos_max < MIN_REL_RHO, se_c < MIN_REL_SE) %>% 
  mutate(z_abs_dev = abs_dev/se_c) %>% 
...
```{r, fig.width=10, fig.height=3, message=FALSE, eval=FALSE}
