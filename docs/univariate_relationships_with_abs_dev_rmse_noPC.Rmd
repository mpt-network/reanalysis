...
all_pairs <- all_pairs %>% 
  ...
  mutate(se_c = rowMeans(cbind(se_x, se_y), na.rm = TRUE)) %>% 
  droplevels()
str(all_pairs)
```
...
We begin by investigating the absolute relationship from the simplest method, the complete pooling MLE method (i.e., `y` always refers to `Comp MLE"` and `x` refers to the other method in the pair). This leaves us with `r nrow(targ_cmle)` observations for the analysis.
...
afex_plot(m_method, "cond_x", 
  ...
  annotate("text", x = 8, y = 0.9, label = 
             paste0("paste(bar(Delta), ' = ", 
                    substr(round(sigma(m_method), 2), 2, 5), "')"), 
                    parse = TRUE) +
...
In case observations had to be removed for the analysis, the percentage of removed (`rem`) observations is also shown in the x-axis caption. In some cases, some observations are filtered to show a clearer picture, this is indicated with `filter`.

### Standard Error

```{r, fig.width=20.5, fig.height=3, message=FALSE}
compare_continuous_covariate(cur_data, se_c)
compare_continuous_covariate(cur_data, se_c, filter = se_c < 1)
compare_continuous_covariate(cur_data, se_c, filter = se_c < 0.25)
...
### Rho (only pairs on same branch)

```{r, fig.width=20.5, fig.height=3, message=FALSE}
compare_continuous_covariate(cur_data, rhos_max)
compare_continuous_covariate(cur_data, rhos_mean)
compare_continuous_covariate(cur_data, rhos_fzmean)
compare_continuous_covariate(cur_data, rhos_med)
compare_continuous_covariate(cur_data, rhos_propl5)
```
...
```{r, fig.width=20.5, fig.height=3, message=FALSE}
compare_continuous_covariate(cur_data, hetero_cohenw)
compare_continuous_covariate(cur_data, hetero_cramerv)
```
...
We can also get this table for the p-value itself (i.e., not `log1p`) which is identical as for very small values `log1p` is approximately equal to `p`.

```{r, message=FALSE}
cur_data %>% 
  group_by(cond_x) %>% 
  summarise(less_than_00001 = mean(p_hetero < 0.00001),
            less_than_01 = mean(p_hetero < 0.01))
```
...
```{r, fig.width=20.5, fig.height=3, message=FALSE}
compare_continuous_covariate(cur_data, chisq_hetero)
compare_continuous_covariate(cur_data, chisq_hetero, 
                             filter = chisq_hetero < 7500)
```
...
### Fungibility (only pairs on same branch)

```{r, fig.width=20.5, fig.height=3, message=FALSE}
compare_continuous_covariate(cur_data, fungis_max)
compare_continuous_covariate(cur_data, fungis_mean)
compare_continuous_covariate(cur_data, fungis_fzmean)
compare_continuous_covariate(cur_data, fungis_med)
compare_continuous_covariate(cur_data, fungis_propl5)
```
...
compare_continuous_covariate(cur_data, fit_cohenw_x)
#compare_continuous_covariate(cur_data, logp_fit_x)
compare_continuous_covariate(cur_data, stat_fit_x)
compare_continuous_covariate(cur_data, stat_fit_z_x)
compare_continuous_covariate(cur_data, stat_fit_scaled_x)
compare_continuous_covariate(cur_data, stat_fit_scaled_x, 
                             filter = stat_fit_scaled_x < 0.5)
compare_continuous_covariate(cur_data, p_fit_y)
compare_continuous_covariate(cur_data, log1p_fit_y)
compare_continuous_covariate(cur_data, fit_cohenw_y)
#compare_continuous_covariate(cur_data, logp_fit_y)
compare_continuous_covariate(cur_data, stat_fit_y)
compare_continuous_covariate(cur_data, stat_fit_z_y)
compare_continuous_covariate(cur_data, stat_fit_scaled_y)
compare_continuous_covariate(cur_data, stat_fit_scaled_y, 
                             filter = stat_fit_scaled_y < 0.2)
```

### Proportion of Individual Misfit 

```{r, fig.width=20.5, fig.height=3, message=FALSE}
compare_continuous_covariate(cur_data, prop_ns_nopb)
compare_continuous_covariate(cur_data, prop_ns_nonpb)
compare_continuous_covariate(cur_data, prop_ns_noasy)
compare_continuous_covariate(cur_data, prop_ns_trait)
```
...
The effect of covariate is shown in two ways. The table below all plots gives the RMSEs for the model given the covariate across all comparison methods. In case the number of levels is not too large, a plot of the difference in absolute deviation conditional on the factor levels is shown. Some factor levels may be removed for plotting (i.e., those levels for which the proportion of observations is less than `r formals(compare_categorical_covariate)$rm_levels_less`). In this case, the number of removed levels is also given.
...
In case observations had to be removed for the analysis, the percentage of removed (`rem`) observations is also shown in the x-axis caption. In some cases, some observationds are filtered to show a clearer picture, this is indicated with `filter`.
